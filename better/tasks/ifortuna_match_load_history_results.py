...
def get_argparse_options():
    ...
    """
    return {
        'description': "One-off job to crawl ifortuna.cz and load history competition results.",
    }
...
class Crawler():
    ...
    def run(self):
        ...
        date = datetime.datetime.strptime(LOAD_TO, '%d.%m.%Y')
        while LOAD_FROM < str(date):
            date = date - datetime.timedelta(days=1)
            logger.info('process of the date ' + str(date))
            ...
            for _segment in _segments:
                s = Segment(self.config['SPORTS'], self.indexer, date)
...
class Segment(HtmlObject):
    def __init__(self, sports, indexer: Indexer, date):
        self.sports = sports
        self.indexer = indexer
        self.date = date
    ...
    def _parse_body(self, tbody_block):
        ...
        for match_row in matches:
            match = Match(self.indexer, self.date)
            try:
...
class Match(HtmlObject):
    def __init__(self, indexer: Indexer, date):
        self.indexer = indexer
        self.date = date
